...
namespace lyramilk{namespace netio
{
	...
	bool aiopoll::add(aioselector* r,uint32 mask)
	{
		...
		if (epoll_ctl(epfd, EPOLL_CTL_ADD, r->getfd(), &ee) == -1) {
			lyramilk::klog(lyramilk::log::error,"lyramilk.system.netio.epoll.add") << lyramilk::kdict("向epoll中添加套接字%d时发生错误%s",r->getfd(),strerror(errno)) << std::endl;
			r->ondestory();
			return false;
		}
		return true;
	...
	bool aiopoll::reset(aioselector* r,uint32 mask)
	{
		...
		if (epoll_ctl(epfd, EPOLL_CTL_MOD, r->getfd(), &ee) == -1) {
			lyramilk::klog(lyramilk::log::error,"lyramilk.system.netio.epoll.reset") << lyramilk::kdict("修改epoll中的套接字%d时发生错误%s",r->getfd(),strerror(errno)) << std::endl;
			return false;
		}
		return true;
	...
	bool aiopoll::remove(aioselector* r)
	{
		...
		if (epoll_ctl(epfd, EPOLL_CTL_DEL, r->getfd(), &ee) == -1) {
			lyramilk::klog(lyramilk::log::error,"lyramilk.system.netio.epoll.remove") << lyramilk::kdict("从epoll中移除套接字%d时发生错误%s",r->getfd(),strerror(errno)) << std::endl;
		}
		r->ondestory();
		return true;
	}
