...
/*
	bool inline yparsearrayblock(yaml_token_t* token,yaml_tokern_stream& z,lyramilk::data::array& ar,lyramilk::data::map& anchor,int deep);

	bool inline yparsemapblock(yaml_token_t* token,yaml_tokern_stream& z,lyramilk::data::map& v,lyramilk::data::map& anchor,int deep)
	{
		while(z.next(token)){

			if(token->type == YAML_BLOCK_ENTRY_TOKEN || token->type == YAML_FLOW_ENTRY_TOKEN){
				if(!z.next(token)){
					return false;
				}
			}
...
COUT << "错误" << std::endl;
			...
			if(!z.next(token) || token->type != YAML_SCALAR_TOKEN){
COUT << "错误" << std::endl;
				return false;
			}
			lyramilk::data::string key = yvalue(token);
			if(!z.next(token) || token->type != YAML_VALUE_TOKEN){
COUT << "错误" << std::endl;
				return false;
			}
...
COUT << "错误" << std::endl;
			...
			if(token->type == YAML_SCALAR_TOKEN){
				lyramilk::data::var value = yvalue(token);
				v[key] = value;
			}else if(token->type == YAML_BLOCK_MAPPING_START_TOKEN || token->type == YAML_FLOW_MAPPING_START_TOKEN){
				lyramilk::data::map value;
				if(!yparsemapblock(token,z,value,anchor,deep+1)){
COUT << "错误" << std::endl;
					return false;
				}
				v[key] = value;
			}else if(token->type == YAML_BLOCK_SEQUENCE_START_TOKEN || token->type == YAML_FLOW_SEQUENCE_START_TOKEN){
				lyramilk::data::array value;
				if(!yparsearrayblock(token,z,value,anchor,deep+1)){
COUT << "错误" << std::endl;
					return false;
				}
				v[key] = value;
			}else if(token->type == YAML_ALIAS_TOKEN){
				lyramilk::data::string alias = (const char*)token->data.alias.value;
				lyramilk::data::var& value = anchor[alias];
				if(value.type() == lyramilk::data::var::t_map){
					lyramilk::data::map& m = value;

					for(lyramilk::data::map::iterator it = m.begin();it!=m.end();++it){
						v[it->first] = it->second;
					}
				}
COUT << "引用锚点" << (const char*)token->data.alias.value << "," << value.type_name() << std::endl;
			}
		}
		return token->type == YAML_BLOCK_END_TOKEN || token->type == YAML_FLOW_MAPPING_END_TOKEN;
	}
	...
	bool inline yparsearrayblock(yaml_token_t* token,yaml_tokern_stream& z,lyramilk::data::array& ar,lyramilk::data::map& anchor,int deep)
	{
		while(z.next(token)){
			...
			if(token->type == YAML_BLOCK_ENTRY_TOKEN || token->type == YAML_FLOW_ENTRY_TOKEN){
				if(!z.next(token)) return token->type == YAML_BLOCK_END_TOKEN || token->type == YAML_FLOW_MAPPING_END_TOKEN;
				if(token->type == YAML_TAG_TOKEN){
					if(!z.next(token)){
						return false;
					}
...
COUT << "处理token" << std::endl;
					lyramilk::data::string anchor = (const char*)token->data.anchor.value;
...
COUT << "错误" << std::endl;
	...
	bool inline yparsedocument(yaml_token_t* token,yaml_tokern_stream& z,lyramilk::data::var& v,lyramilk::data::map& anchor,int deep)
	{
		while(z.next(token)){
			...
			if(token->type == YAML_BLOCK_MAPPING_START_TOKEN || token->type == YAML_FLOW_MAPPING_START_TOKEN){
				v.type(lyramilk::data::var::t_map);
				if(!yparsemapblock(token,z,v,anchor,deep+1)){
COUT << "错误" << std::endl;
					return false;
				}
			}else if(token->type == YAML_BLOCK_SEQUENCE_START_TOKEN || token->type == YAML_FLOW_SEQUENCE_START_TOKEN){
				v.type(lyramilk::data::var::t_array);
				if(!yparsearrayblock(token,z,v,anchor,deep+1)){
COUT << "错误" << std::endl;
					return false;
				}
			}
		}
		return token->type == YAML_DOCUMENT_END_TOKEN;
	}
	...
	bool inline yparseroot(yaml_token_t* token,yaml_tokern_stream& z,lyramilk::data::array& ar,lyramilk::data::map& anchor,int deep)
	{
		while(z.next(token)){
			if(token->type == YAML_DOCUMENT_START_TOKEN){
				lyramilk::data::var v;
				ar.push_back(v);
				if(!yparsedocument(token,z,ar.back(),anchor,deep+1)){
COUT << "错误" << std::endl;
					return false;
				}
			}else if(token->type == YAML_BLOCK_MAPPING_START_TOKEN || token->type == YAML_FLOW_MAPPING_START_TOKEN){
				...
				if(!yparsemapblock(token,z,ar.back(),anchor,deep+1)){
COUT << "错误" << std::endl;
					return false;
				}
			}else if(token->type == YAML_BLOCK_SEQUENCE_START_TOKEN || token->type == YAML_FLOW_SEQUENCE_START_TOKEN){
				//	array 类型
				ar.push_back(lyramilk::data::array());
				if(!yparsearrayblock(token,z,ar.back(),anchor,deep+1)){
COUT << "错误" << std::endl;
					return false;
				}
			}
